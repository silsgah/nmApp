...
import {
  ...
  ChevronRight, ChevronLeft, ChevronDown, Bell, Menu, X, GraduationCap, Download, FileSpreadsheet, ShieldCheck, FileText, HelpCircle
} from "lucide-react";
...
const navItems: NavItem[] = [
  { href: "/admin", label: "Dashboard", icon: LayoutDashboard, exact: true },
  {
    ...
    icon: ShieldCheck,
    children: [
      { href: "/admin/programmes", label: "Check Programs", icon: GraduationCap },
      { href: "/admin/students", label: "Add Candidate", icon: GraduationCap },
      { href: "/admin/users", label: "Add Examiners", icon: Users },
      { href: "/admin/tasks", label: "View Tasks", icon: BookOpen },
    ],
  },
  {
    label: "Examinations",
  ...
  },
  {
    ...
    icon: Download,
    children: [
      { href: "/admin/results", label: "Practical Results", icon: BarChart3 },
      { href: "/admin/case-studies", label: "Care Study", icon: FileText },
      { href: "/admin/care-plans", label: "Care Plan", icon: Stethoscope },
      { href: "/admin/obstetric", label: "Obstetrician Result", icon: Stethoscope },
    ],
  },
  ...
  const SidebarContent = ({ compact = false }: { compact?: boolean }) => (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-1">
        <p className={cn("text-xs font-semibold text-sidebar-foreground/40 uppercase tracking-wider px-3 py-2", compact && "sr-only")}>
          Management
        </p>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {!compact && isActive(item.href!, item.exact) && (
                <ChevronRight className="w-3 h-3 opacity-60 flex-shrink-0" />
              )}
            </Link>
          );
        ...
        >
          ...
          {!compact && <span className="flex-1 truncate">Admin Guide & Lessons</span>}
        </button>
      </nav>
